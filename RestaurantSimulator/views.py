...
def index(request):
    ...
    if chat_id := request.GET.get('chat_id'):
        if chat_thread := models.SimulatedChatThread.objects.filter(id=chat_id).first():
            waiter_tokens_used = get_total_tokens_used(chat_thread, models.RoleType.WaiterBot)
            customer_tokens_used = get_total_tokens_used(chat_thread, models.RoleType.CustomerBot)
